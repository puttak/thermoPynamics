...
class ReadDataUNIFAC:
    ...
    def GetGroup_interaction_parameter(self):
        ...
        b_m_n = np.zeros([self.NG, self.NG])
        c_m_n = np.zeros([self.NG, self.NG])
        T0=np.zeros([self.NG,self.NG])
        self.cursorUNIFAC.execute('SELECT A_i_j, A_j_i, B_i_j, B_j_i, C_i_j, C_j_i, T0 '
                                  'FROM modInterParamUNIFAC WHERE groupID_i=%d and groupID_j=%d' % (1, 42))
        row=self.cursorUNIFAC.fetchall()
        ...
        for sg_i in self.k:
            ...
            for sg_j in sg_j_list:
                ...
                try:
                    ...
                    self.cursorUNIFAC.execute('SELECT A_i_j, A_j_i, B_i_j, B_j_i, C_i_j, C_j_i, T0 FROM modInterParamUNIFAC WHERE '
                    ...
                    b_m_n[self.k.index(sg_i)][self.k.index(sg_j)] = row[0][2]
                    b_m_n[self.k.index(sg_j)][self.k.index(sg_i)] = row[0][3]
                    c_m_n[self.k.index(sg_i)][self.k.index(sg_j)] = row[0][4]
                    c_m_n[self.k.index(sg_j)][self.k.index(sg_i)] = row[0][5]
                    T0[self.k.index(sg_i)][self.k.index(sg_j)] = row[0][6]
                    T0[self.k.index(sg_j)][self.k.index(sg_i)] = row[0][6]
                except:
                    try:
                        self.cursorUNIFAC.execute('SELECT A_i_j, A_j_i, B_i_j, B_j_i, C_i_j, C_j_i FROM modInterParamUNIFAC '
